...
export function LinksManager() {
  ...
  const categoryOptions = useMemo(() => ["All", ...data.categories.map((category) => category.name)], [data.categories]);
  const visibleLinks = data.links
    .filter((link) => filter === "All" || link.categoryName === filter)
  ...
  return (
    <div className="space-y-6">
      <section className="glass-card rounded-[2rem] p-5">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-1 items-center gap-3 rounded-2xl bg-white/60 px-4 py-3 dark:bg-white/5">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search links..." className="w-full border-0 bg-transparent p-0 focus:ring-0" />
          </div>
      ...
      {editing && (
        <EditorModal title={editing.id ? "Edit Link" : "Add Link"} onClose={() => setEditing(null)} onSubmit={submit}>
...
function LinkFields({ link, categories, onChange }: { link: PortalLink; categories: Category[]; onChange: (link: PortalLink) => void }) {
  return (
    <div className="grid gap-4 md:grid-cols-2">
      <Input label="Title" value={link.title} onChange={(title) => onChange({ ...link, title })} />
      <Input label="Icon" value={link.icon} onChange={(icon) => onChange({ ...link, icon })} />
      <Input label="URL" value={link.url} onChange={(url) => onChange({ ...link, url })} className="md:col-span-2" />
      ...
      <Input label="Sort Order" value={String(link.sortOrder)} onChange={(sortOrder) => onChange({ ...link, sortOrder: Number(sortOrder) })} />
